  ...
  persist(
    (set, get) => ({
      ...
      nextStep: () => {
        const state = get();
        if (state.currentEvaluation) {
          const maxSteps = state.isDevilsAdvocateMode ? 5 : 4; // Summary is the last logical step before success screen
          const nextStep = Math.min(state.currentEvaluation.currentStep + 1, maxSteps + 1);
