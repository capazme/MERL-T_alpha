...
3. **Backup System**
   ...
   - Stored in `merlt/rlcf_framework/config_backups/`
...
**Location:** `merlt/rlcf_framework/config_manager.py`
...
```bash
# Edit merlt/rlcf_framework/task_config.yaml
...
```bash
# Manual backup
cp merlt/rlcf_framework/task_config.yaml task_config_backup_$(date +%Y%m%d_%H%M%S).yaml
```
