...
```bash
# Apri l'editor
code backend/rlcf_framework/task_config.yaml
...
```bash
# Modifica backend/rlcf_framework/task_config.yaml
...
- 📁 `backend/rlcf_framework/config_backups/`
...
# 2. Controlla i log
tail -f backend/rlcf_detailed.log | grep ConfigManager
...
3. **Monitora i logs** durante lo sviluppo:
   ```bash
   tail -f backend/rlcf_detailed.log | grep -E "ConfigManager|RLCF"
   ```
